package com.czxy.BlackList.tag

import java.util.Properties

import com.czxy.BlackList.bean.{HBaseMeta, TagRule}
...
object BlackListTag {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //3、读取四级标签数据
    val fourTagsDS: Dataset[Row] = mysqlConn.select("rule").where("id=71")
    ...
    //4、读取五级标签数据
    val fiveTagsDS: Dataset[Row] = mysqlConn.select("id", "rule").where("pid=71")
    //封装成 TagRule 样例类
    ...
    //5、读取hbase数据
    val hbaseDatas: DataFrame = spark.read.format("com.czxy.BlackList.tools.HBaseDataSource")
    ...
    //id job
    val jobNewTags: DataFrame = hbaseDatas.select('id.as("userId"), getTags('is_blackList).as("tagsId"))
    //输出 查看效果
    //+------+------+
    //|userId|tagsId|
    //+------+------+
    ...
    val oldTags: DataFrame = spark.read.format("com.czxy.BlackList.tools.HBaseDataSource")
      .option(HBaseMeta.ZKHOSTS, hbaseMeta.zkHosts) //192.168.10.20
      .option(HBaseMeta.ZKPORT, hbaseMeta.zkPort) //2181
    ...
    //c、最后覆盖写入hbase
    //8、将最终数据写入hbase
    allTags.write.format("com.czxy.BlackList.tools.HBaseDataSource")
